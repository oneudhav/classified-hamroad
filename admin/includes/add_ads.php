							   ...
							   <form class="form-group" method="post"  enctype="multipart/form-data" action="">
									`<div id="imagePreview"></div>
										<div class="form-group">
											<input class="form-control"id="uploadFile" type="file" name="upload" placeholder="select item photo">
										</div>
										 <div class="form-group">
											<label>Select Username</label><br/>
											<select class="form-control" name="user" onchange="changeContent(this.value)">
												<option></option>
												...
												while ($ary = mysqli_fetch_array($result)){
													echo "<option value=\"" . $ary['user']  . "\">" . $ary ['user']  . "</option>";
												}
												?>
											</select>
										</div>
										<div  class="form-group">
										 ...
										 <div class="form-group">
											<input class="form-control"type="text" name="prize" placeholder="* Price">
										 </div>
										<div class="form-group">
											<textarea style="resize:none;" class="form-control"type="text" cols="41" rows="6"name="desc" placeholder="* Description"></textarea>
										</div>
									   <div class="form-group">
											<label>Category</label><br/>
											<select class="form-control" name="cat" onchange="changeContent(this.value)">
												<option></option>
											...
											<div class="form-group">
											 ...
											 <select class="form-control" name="subcat" id="options">
											 </select>
											 </div>
												<div class="form-group">
													<button class="btn btn-success" type="submit" name="sell">Post Advert</button>
												</div>
												<?php
													if(isset($_POST['sell']))
												{
												...
												$user = stripslashes($user);
												 ...
												 $ads_id= randomkey(30);
													...
													if($title =="" || $desc=="" || $prize=="")
													{
														echo "<br/>Title,Description and Price is empty";
													}
													else
													{
												//
														...
														{
															$max_size = 5000; //max image size in Pixels
																						...
																						if(imagecopyresampled($new_canvas, $image_resource , 0, 0, 0, 0, $new_image_width, $new_image_height, $img_width, $img_height))
																							{
																								...
																								//center watermark
																								   ...
																								   $image_name=$ads_id;
																									...
																									$phone=$rows['phone'];
																									$insert = "INSERT INTO sell (id,ads_id,user,item_pix,title,description,prize,name,phone,category,sub_category)
																										VALUES ('$id','$ads_id','$user','$photo','$title','$desc','$prize','$name','$phone','$cat','$subcat')";
																										mysqli_query($con,$insert) or die('Cannot add record '.mysqli_error());
																										
																											   
																												echo "Advertisment Posted";
